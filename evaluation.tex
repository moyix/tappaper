...
is perhaps hardly surprising that the correspondence is poor.  

There is some hope, however.  
Regardless of the apparently poor clustering performance with respect to 
hand-labeling, we decided to determine if the clusters contained new and 
interesting data and if finding that data would be facilitated by them.  
First, we determined to which of the 100 clusters data from the dmesg 
and filename tap points was assigned.  
We were heartened to learn that all of these two text-like tap points 
...
graphviz files, and so on made an appearance.  
...
as it appears to contain much of interst for active monitoring.
